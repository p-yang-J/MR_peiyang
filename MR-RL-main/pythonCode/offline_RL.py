...
def getdata():
    ...
    ##offline RL 参数
    observations = np.zeros((1000, 6))
    actions = np.zeros((1000, 3))
    ...
    for i in range(1000):
        ...
        if first_run or cluster:
            ...
            print("效率：", effiency)
            #print("时间：", timing)
            reward = (-G - S)*15 + (200000-effiency)/5000
            endtime = time.time()
            ...
            rewards[i] = reward
            observations[i] = [addx, addy, addz, scale_ada_x, scale_ada_y, scale_ada_z]
            actions[i] = [scale_addx, scale_addy, scale_addz]
...
def evl():
    ...
    dataset = d3rlpy.dataset.MDPDataset(
        observations=np.load('observations4.npy'),
        actions=np.load('actions4.npy'),
        rewards=np.load('rewards4.npy'),
        terminals=np.load('terminals4.npy'),
    )
    dqn2 = d3rlpy.algos.CRR()
    dqn2.build_with_dataset(dataset)
    dqn2.load_model('bcq_test4')
    ...
    for i in range(1000):
        ...
        if first_run or cluster:
            ...
            deulerX_filtered = deulerX_filter.update(deulerX)
            points_hand.append([dx_filtered, dy_filtered, dz_filtered])
            x_batch = [addx, addy, addz, scale_ada_x, scale_ada_y, scale_ada_z]
            ...
            #
            observation = [addx, addy, addz, scale_ada_x, scale_ada_y, scale_ada_z]
            human_feed = super_model.predict(np.array(observation).reshape(1, -1))
            print("自适应动作",actions_original)
            #
            scale_ada_x = 6000 + scale_addx + 10000 * 1 / (1 + np.exp(-abs(x_dist) / 500)) + int(human_feed*8)
            scale_ada_y = 6000 + scale_addy + 10000 * 1 / (1 + np.exp(-abs(y_dist) / 500)) + int(human_feed*8)
            scale_ada_z = 6000 + scale_addz + 10000 * 1 / (1 + np.exp(-abs(z_dist) / 500)) + int(human_feed*8)
...
def train():
    ...
    dqn = d3rlpy.algos.CQL(use_gpu=False)
    ...
    for i in range(1000):
        ...
        if first_run or cluster:
            ...
            x_array = np.array(x_batch)
            x_array = x_array.reshape(1, -1)
            actions_new = dqn.predict(x_array)[0]
            print("训练得到的action：",actions_new)
            scale_addx = actions_new[0]
            scale_addy = actions_new[1]
            scale_addz = actions_new[2]
            scale_ada_x = 11000 + abs(x_dist) * 5 + scale_addx
            scale_ada_y = 11000 + abs(y_dist) * 5 + scale_addy
            scale_ada_z = 11000 + abs(z_dist) * 5 + scale_addz
            scale1_list.append(scale_ada_x)
            scale2_list.append(scale_ada_y)
            scale3_list.append(scale_ada_z)
            disx = dx_filtered * scale_ada_x
