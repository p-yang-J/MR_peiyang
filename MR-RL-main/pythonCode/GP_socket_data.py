...
while True:
   ...
   #print('Received message:', data.decode('utf-8'))  # 将接收到的字节数据解码成字符串并打印
    ...
    if first_run:
        ...
        startPos[0] += (dz/(7+beta_1))  # increase z by one
        startPos[1] += (dx/(7+beta_1))
        startPos[2] += (dy/(7+beta_1))
        ...
        if len(points) >= 10:  # Replace 4 with the actual number of points you need
            ...
            e1 = 0
            e2 = 0
            value1 = 1 / (1 + np.exp(S - e1))
            value2 = 1 / (1 + np.exp(G - e2))
            beta_1 = max(value1,value2)
            print("G:", G)
            print("S:", S)
            print("beta1:",beta_1)
